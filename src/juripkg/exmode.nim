...
proc staticReadVersionFromConfigFileExample(): string {.compileTime.} =
  staticRead(currentSourcePath.parentDir() / "../../example/jurirc.toml")
...
proc putConfigFileCommand(status: var Editorstatus) =
  ...
  if not dirExists(getHomeDir() / ".config" / "juri"):
    try:
      createDir(getHomeDir() / ".config" / "juri")
    except OSError:
      status.commandLine.writePutConfigFileError(status.messageLog)
      status.changeMode(currentBufStatus.prevMode)
      return

  if fileExists(getHomeDir() / ".config" / "juri" / "jurirc.toml"):
    status.commandLine.writePutConfigFileAlreadyExistError(status.messageLog)
    status.changeMode(currentBufStatus.prevMode)
    return

  let path = getHomeDir() / ".config" / "juri" / "jurirc.toml"
